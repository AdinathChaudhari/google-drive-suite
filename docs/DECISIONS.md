...
- **Hit** — on the Fire TV Stick, opening **Blackadder**, stepping LEFT off the
  episode list onto the season list, and picking Season 1 killed the app every
  single time. Movies were fine, and so was every show the user happened to try,
  which made it look like bad metadata for one title. It wasn't: `/api/title` for
  Blackadder is clean (5 seasons, 39 episodes, no duplicate `file_id`s, no nulls).
- **Learned** — `Modifier.focusRestorer` (wrapped as `tvFocusRestorer`) does more
  ...
  **Blackadder is not special — its metadata is.** Its Season 0 of 15 specials is
